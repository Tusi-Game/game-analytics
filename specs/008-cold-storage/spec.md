# Cold-Storage Lifecycle (US5)

**Part of:** [001-analytics-platform](../001-analytics-platform/spec.md)
**Story:** US5 (P3 — operational) · **Reserved kind owned:** none (ops) · **Status:** Draft
**Depends on:** [002-foundation-ingest](../002-foundation-ingest/spec.md) (the raw event stream workers append; read anytime after ingest). Consumes the quarantine tails of [004-economy](../004-economy/spec.md) / [005-retention](../005-retention/spec.md) / [006-monetization](../006-monetization/spec.md) (typed-invalid and sealed-late events). Config + erasure ledger from [011-operator-admin](../011-operator-admin/spec.md).

**Deeper reference:** no dedicated metric sheet — this is an operational story. **Spec anchors:** US5, FR-023/024, SC-010, and the Redis-loss / write-ahead Edge Cases in [001-analytics-platform](../001-analytics-platform/spec.md). The normative append/lifecycle contract is pinned in [bridge 01.5](../001-analytics-platform/bridges/01.5-raw-file-contract.md).
**Excluded here** (→ design.md and later per-story design): file format, compression codec, S3 client, scheduler, key layouts.
...
- **During 2026-07-16 (UTC):** workers append every accepted raw event for `g=42` — write-ahead, before counters — into `g42/2026-07-16.log.gz`. The 5 malformed `economy` events from the [002-foundation-ingest](../002-foundation-ingest/spec.md) example also land here (quarantine tail). By midnight the file is a complete superset of everything counted that day.
...
This story consumes the **same raw event stream** [002-foundation-ingest](../002-foundation-ingest/spec.md) ingests — it adds **no new SDK field**. What it needs is not from the SDK but from the pipeline and config:
...
- **Quarantine-flagged events** — typed-invalid or sealed-late events the metric stories refuse — routed to the same file's quarantine tail.
...
Cold storage is deliberately the story that stores **the most and keeps it the least**:
...
- **No database rows.** This story writes **nothing** durable to the results database — that would violate results-only (FR-010). The database never holds raw events; the file is the only raw form and it is explicitly disposable.
...
The whole point: raw truth is voluminous, so it is kept **off the hot path and out of the database**, compressed, shipped, and deleted — the opposite storage posture from every metric story.
...
**Redis (transient).**
- **No cold-storage state in Redis.** Redis holds the queue + hot counters ([002-foundation-ingest](../002-foundation-ingest/spec.md)); the raw file is written to **local disk** by the worker, not to Redis. The only interaction is *ordering*: the fsync'd file append happens **before** the Redis counter update (write-ahead), so a Redis loss never leaves a counted-but-unlogged event.
...
**Inherited globals (referenced):** the write-ahead ordering and 48 h day-seal / quarantine rules are pipeline invariants (owned by [002-foundation-ingest](../002-foundation-ingest/spec.md) / §E / §G), not cold-storage knobs — cold storage honors them but does not define them. All operational choices here are **configurable so the operator can change strategy without code changes** (FR-027).

---

## Cross-references

- No metric sheet (operational story). Grounded in [001-analytics-platform](../001-analytics-platform/spec.md): US5, FR-023/024, SC-010, and the Redis-loss / write-ahead / quarantine Edge Cases.
- **Upholds the durability invariant** every metric story relies on (write-ahead raw append → complete superset → SC-008). Consumes [002-foundation-ingest](../002-foundation-ingest/spec.md)'s raw stream and the quarantine tails of [004-economy](../004-economy/spec.md) / [005-retention](../005-retention/spec.md) / [006-monetization](../006-monetization/spec.md) (typed-invalid and sealed-late events).
- **Future escape hatch:** the raw file is the theoretical source for the deferred "rebuild dimensions forward from raw" backfill and the manual raw-file rebuild runbook ([research §X-1](../001-analytics-platform/research.md)) — neither shipped in v1.

---

*Design (ER / data model, Redis structures, worker/pipeline flow, API/contract surface, cross-story relations) lives in [design.md](./design.md).*
