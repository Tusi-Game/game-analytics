...
**SCOPE NOTE — supersedes FR-022's "design-only" posture.** FR-022 currently defers funnels to a forward-compatible *data-model design only* ("no v1 funnel work"). This sheet **promotes funnels to a fully-specified, computed metric** with real calculation and config, at the operator's explicit request. That promotion is a **scope decision the operator must ratify** — it is not implied by FR-022 and it directly contradicts the "Funnels are deferred" clarification (spec §Clarifications, 2026-07-17). The tension is real and load-bearing: **funnels are the single hardest metric to compute under results-only storage**, because a funnel is inherently *per-user sequential path data* ("did THIS user do A then B then C in-window"), and the platform forbids raw event logs in Postgres and caps Redis at ≤1 day. Promoting funnels therefore **expands the durable per-user spine** (see §5), which trades directly against SC-007 (spine stays tiny). This decision is flagged again in §7. Do not read this sheet as having silently overridden FR-022; it proposes overriding it and asks for ratification.
...
- **§OQ-1 — RATIFY PROMOTING FUNNELS TO v1 (scope decision, highest-leverage) [OPEN]**: This sheet supersedes FR-022's "design-only, deferred" posture and the 2026-07-17 clarification that "Funnels are deferred." Promoting funnels to a computed v1 metric adds a **per-user progress spine** that trades against SC-007 (§5). The operator must explicitly ratify this scope change before /plan, or the sheet reverts to design-only. *Recommendation: ratify a **minimal** v1 funnel — single per-game funnel, `sequential-with-others-allowed`, first-touch, furthest-step-reached histogram + overall time-to-convert only — accepting the bounded spine cost and pruning progress rows once cohorts seal. Defer strict/any-order/per-step-timing/any-touch.*
